-- Dnes: odpověď rovnou z hlavní plochy. Akce „Odpovědět“ vede na
-- `/posta?polozka=<id>&odpovedet=1` — Pošta zprávu otevře a rovnou rozbalí
-- okno odpovědi s návrhem z běhu. Nová kategorie `koncept`: položky P2 ve stavu
-- `nove`, ke kterým běh připravil návrh (`navrh_telo`); P1 mají Odpovědět jako
-- druhou akci. Jen dopředná změna, tvar výstupu je stejný (Signal v today.ts).
...
as $$
  ...
  p1 as (
    select
      ...
      case when p.navrh_telo is not null and p.stav in ('nove', 'ceka')
        then jsonb_build_array(
          jsonb_build_object('popisek', 'Odpovědět', 'ikona', 'reply', 'href', '/posta?polozka=' || p.id || '&odpovedet=1'),
          jsonb_build_object('popisek', 'Otevřít', 'ikona', 'mail', 'href', '/posta?polozka=' || p.id))
        else jsonb_build_array(jsonb_build_object('popisek', 'Otevřít', 'ikona', 'mail', 'href', '/posta?polozka=' || p.id))
      end as akce,
      null::date as termin,
  ...
  koncept as (
    select
      'koncept:' || p.id as klic,
      3::smallint as urg,
      'koncept' as kat,
      coalesce(p.predmet, '(bez předmětu)') as nazev,
      coalesce(nullif(split_part(p.co_resit, E'\n', 1), ''), 'Návrh odpovědi je připravený') as proc,
      array_remove(array[
        coalesce(p.od, p.od_email),
        case when p.datum is not null then 'přišlo ' || to_char(p.datum, 'FMDD. FMMM.') end
      ], null) as dukazy,
      jsonb_build_array(
        jsonb_build_object('popisek', 'Odpovědět', 'ikona', 'reply', 'href', '/posta?polozka=' || p.id || '&odpovedet=1'),
        jsonb_build_object('popisek', 'Otevřít', 'ikona', 'mail', 'href', '/posta?polozka=' || p.id)) as akce,
      null::date as termin,
      '/posta?polozka=' || p.id as href
    from doktor.polozky p
    where p.priorita = 2 and p.stav = 'nove' and p.navrh_telo is not null
    order by p.datum desc nulls last
    limit 20
  ),
  odpoved as (
    select
      ...
      jsonb_build_array(jsonb_build_object('popisek', 'Odpovědět', 'ikona', 'reply', 'href', '/posta?polozka=' || p.id || '&odpovedet=1')) as akce,
      null::date as termin,
      '/posta?polozka=' || p.id as href
    from doktor.polozky p
